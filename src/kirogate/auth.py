"""KiroGate authentication module.
...
_TOKEN_ENV_VAR = "KIROGATE_AGENT_TOKEN"
...
def authenticate_caller(provided_token: str) -> bool:
    ...
    Returns:
        ...
        stored in the KIROGATE_AGENT_TOKEN environment variable.
...
def validate_startup() -> None:
    ...
    Raises SystemExit with a non-zero exit code if KIROGATE_AGENT_TOKEN
    ...
    if not token:
        print(
            ...
            "KiroGate refuses to start without a configured agent token.",
            file=sys.stderr,
        )
        raise SystemExit(1)
