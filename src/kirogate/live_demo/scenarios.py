"""Live demo scenarios — real AI agent actions through KiroGate.
...
The flow for each:
    User intent → LLM generates action → KiroGate evaluates → Allow/Deny
"""
...
from kirogate.live_demo.database import execute_query
from kirogate.live_demo.llm_provider import LLMProvider, get_provider
...
async def _scenario_read_customers(
    ...
    return ScenarioResult(
        ...
        "KiroGate allows SELECT operations and redacts sensitive fields (SSN) from the response.",
...
async def _scenario_delete_data(
    ...
    return ScenarioResult(
        ...
        "KiroGate blocks the DELETE operation — only SELECT is permitted by policy. "
...
async def _scenario_ssrf_attempt(
    ...
    return ScenarioResult(
        ...
        "to steal IAM credentials. KiroGate's egress control blocks this — the metadata IP "
...
async def _scenario_exfiltrate_data(
    ...
    return ScenarioResult(
        ...
        "to an external attacker-controlled endpoint. KiroGate's egress control blocks "
