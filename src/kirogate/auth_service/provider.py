...
class LocalAuthProvider:
    ...
    Environment variables:
        KIROGATE_OPERATOR_EMAIL: expected operator email
        KIROGATE_OPERATOR_PASSWORD: expected operator password (hashed in prod)
        KIROGATE_OPERATOR_WORKSPACE: allowed workspace name
    """

    def authenticate(self, workspace: str, email: str, password: str) -> UserInfo | None:
        expected_email = os.environ.get("KIROGATE_OPERATOR_EMAIL", "admin@kirogate.dev")
        expected_password = os.environ.get("KIROGATE_OPERATOR_PASSWORD", "kirogate-demo")
        expected_workspace = os.environ.get("KIROGATE_OPERATOR_WORKSPACE", "kirogate")
