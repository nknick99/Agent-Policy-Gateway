"""KiroGate CLI — the simplest way to protect any MCP server.

Usage:
    kirogate proxy --target http://localhost:9000 --policy policy.json
    kirogate proxy --target http://localhost:9000  # uses default deny-all policy
    kirogate demo   # runs a built-in demo showing allow/deny
...
def main():
    ...
    proxy_parser = subparsers.add_parser(
        "proxy",
        help="Start KiroGate as a transparent proxy in front of any MCP server",
    )
    ...
    proxy_parser.add_argument(
        ...
        type=int,
        default=8000,
        help="Port for KiroGate to listen on (default: 8000)",
    )
    proxy_parser.add_argument(
        "--token",
        default=None,
        help="Agent bearer token (default: from KIROGATE_AGENT_TOKEN env var)",
    )
    proxy_parser.add_argument(
        "--audit-file",
        default="kirogate-audit.jsonl",
        help="Audit log file path (default: kirogate-audit.jsonl)",
    )
    proxy_parser.add_argument(
        "--mode",
    ...
    subparsers.add_parser(
        "demo",
        help="Run an interactive demo showing KiroGate enforcement",
    )
...
def _run_proxy(args):
    ...
    # Set token
    if args.token:
        os.environ["KIROGATE_AGENT_TOKEN"] = args.token
    elif not os.environ.get("KIROGATE_AGENT_TOKEN"):
        # Generate a random token for easy local testing
        token = hashlib.sha256(str(time.time()).encode()).hexdigest()[:32]
        os.environ["KIROGATE_AGENT_TOKEN"] = token
        print(f"  Generated token: {token}")
        print(f"  Set KIROGATE_AGENT_TOKEN={token} in your agent's env")
        print()
    ...
    if not policy_path.exists():
        ...
        print(f"  Run 'kirogate init' to generate a starter policy")
        print(f"  Or KiroGate will use default deny-all policy")
        print()

    os.environ["KIROGATE_TARGET_URL"] = args.target
    os.environ["KIROGATE_AUDIT_FILE"] = args.audit_file
    os.environ["KIROGATE_MODE"] = args.mode

    print("┌─────────────────────────────────────────────┐")
    print("│         KiroGate Policy Gateway              │")
    print("├─────────────────────────────────────────────┤")
    print(f"│  Listening:  http://0.0.0.0:{args.port}         │")
    print(f"│  Target:     {args.target:<30}│")
    print(f"│  Policy:     {args.policy:<30}│")
    print(f"│  Mode:       {args.mode:<30}│")
    print(f"│  Audit log:  {args.audit_file:<30}│")
    print("├─────────────────────────────────────────────┤")
    print("│  Point your MCP client here instead of the  │")
    print("│  target. KiroGate will enforce policy on     │")
    print("│  every tools/call request.                   │")
    print("└─────────────────────────────────────────────┘")
    print()

    import uvicorn
    from kirogate.proxy_app import create_proxy_app
...
def _run_demo():
    ...
    print("┌─────────────────────────────────────────────┐")
    print("│       KiroGate Live Demo                     │")
    print("└─────────────────────────────────────────────┘")
    print()
    print("This simulates an AI agent making requests through KiroGate.")
    print()

    # Inline demo — no server needed
    from kirogate.proxy_app import evaluate_request
    ...
    print("─────────────────────────────────────────────")
    print("KiroGate blocked 3/4 requests. The one allowed request")
...
def _run_init(args):
    ...
    print("Then run:")
    print(f"  kirogate proxy --target http://your-mcp-server:9000 --policy {output_path}")
