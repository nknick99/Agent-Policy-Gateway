...
tries to access through Agent Policy Gateway's policy gateway.
"""
...
def _seed_data(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript("""
        ...
        VALUES
            (1, 1, 'Agent Policy Gateway Enterprise', 2400.00, 'active'),
            (2, 2, 'Agent Policy Gateway Pro', 49.00, 'active'),
            (3, 3, 'Agent Policy Gateway Enterprise', 2400.00, 'active'),
            (4, 1, 'Support Add-on', 500.00, 'pending'),
            (5, 5, 'Agent Policy Gateway Pro', 49.00, 'active');
...
def execute_query(sql: str) -> list[dict]:
    ...
    AFTER Agent Policy Gateway has already approved the request.
    """
    conn = get_connection()
