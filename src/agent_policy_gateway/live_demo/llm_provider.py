...
wants to do). Agent Policy Gateway then evaluates that intent against policy BEFORE
it ever executes.
...
Architecture for multi-agent systems:
    ...
    └────┬─────┘     └────┬─────┘     └────┬─────┘
         ...
         └────────────────┼────────────────┘
                          │
                    ┌─────▼─────┐
                    │  Agent Policy Gateway │  ← Deterministic policy enforcement
...
Agent Policy Gateway doesn't depend on the LLM provider. It only cares about
