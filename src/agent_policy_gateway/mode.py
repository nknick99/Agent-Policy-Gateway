"""Operating mode determination and control for Agent Policy Gateway Proxy.
...
Mode is determined once at startup from the APG_MODE environment
...
def get_operating_mode() -> OperatingMode:
    ...
    if mode_str:
        logger.warning(
            "Invalid APG_MODE value '%s', defaulting to Enforce mode",
            mode_str,
        )
    return OperatingMode.ENFORCE
...
class ModeController:
    ...
    def __init__(self) -> None:
        ...
        logger.info("Agent Policy Gateway operating in %s mode", self._mode.value.upper())
